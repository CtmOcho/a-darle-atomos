...
def calibrate_camera_for_intrinsic_parameters(images_prefix):
    ...
    #NOTE: images_prefix contains camera name: "frames/c0*".
...
def select_frames(prefix):
    c0_images = sorted(glob.glob(os.path.join(prefix, 'c0_*.png')))
    c1_images = sorted(glob.glob(os.path.join(prefix, 'c1_*.png')))

    selected_frames = []

    for frame_c0, frame_c1 in zip(c0_images, c1_images):
        img_c0 = cv.imread(frame_c0)
        img_c1 = cv.imread(frame_c1)
...
if __name__ == '__main__':
    ...
    a = False
    k = input("Para capturar nuevos frames individuales ingrese la tecla ESPACIO")
    if k == ' ':
        a = True    
    ...
    #check_calibration('c0', camera0_data, 'c1', camera1_data, _zshift = 80.)


    #"""Optional. Define a different origin point and save the calibration data"""
    c0_frame, c1_frame = select_frames('frames_pair') 

    ##get the world to camera0 rotation and translation
    R_W0, T_W0 = get_world_space_origin(cmtx0, dist0, c0_frame)
    ...
    save_extrinsic_calibration_parameters(R_W0, T_W0, R_W1, T_W1) #this will write R and T to disk
