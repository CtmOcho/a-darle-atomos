...
def read_keypoints(filename):
    ...
    while(True):
        ...
        line = np.reshape(line, (42, -1))  # Ajuste para 42 puntos (2 manos)
        kpts.append(line)
...
def visualize_3d(p3ds):
    ...
    for i, kpts3d in enumerate(p3ds_rotated):
        ...
        for hand_offset in [0, 21]:  # Procesar ambas manos
            for finger, finger_color in zip(fingers, fingers_colors):
                for _c in finger:
                    ax.plot(xs=[kpts3d[_c[0] + hand_offset, 0], kpts3d[_c[1] + hand_offset, 0]], 
