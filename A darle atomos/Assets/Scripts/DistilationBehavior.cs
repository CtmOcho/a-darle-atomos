...
public class DistilationBehavior : MonoBehaviour
{
        ...
        foreach (Rigidbody rb in particleRigidbodies)
        {
            ...
            if (isMaxTemperatureReached)
            {
                vibration = new Vector3(
                    ...
                    Mathf.Abs(Random.Range(0f, 1f)) * vibrationIntensity * Time.deltaTime * 1.2f, // Sesgo más fuerte hacia el eje Y positivo
                    Random.Range(-1f, 1f) * vibrationIntensity * Time.deltaTime
                );
            }
            else
